...
logfiles_PC9 <- c("TAXA_16_PCs_9",
                  "TAXA_32_PCs_9",
                  "TAXA_60_PCs_9",
                  "TAXA_87_PCs_9")

logfiles_PC9 <- c("TAXA_16_PCs_2",
                  "TAXA_16_PCs_3",
                  "TAXA_16_PCs_6",
                  "TAXA_16_PCs_9",
                  "TAXA_16_PCs_10",
...
list_of_plots <- list()
for(i in logfiles_PC9){
  
  current_logfile_path <- paste0("2_scripts/new_xmls/done/", i, "/independent_run_1/output/FBD_skyline.log")
  ...
  if(file.exists(current_logfile_path)){
    ...
    deathRates$Event <- c("GS-2", "GI-1", "GS-1", "Holocene")
    ...
    birthRates$Event <- c("GS-2", "GI-1", "GS-1", "Holocene")
    ...
    list_of_plots[[i]] <- 
      rates %>% 
      ggplot2::ggplot() +
      ggplot2::geom_point(aes(x = Event,
                              y = med)) +
      ggplot2::geom_errorbar(aes(x = Event,
                                 ymin = lower, ymax = upper),
                             width = 0.1) +
      ggplot2::geom_line(aes(x = Event,
                             y = med,
                             group = item)) +
      facet_wrap(~item,
                 scales = "free",
...
logfile_rates_name_type %>% 
  ggplot(data = .) +
  geom_boxplot(aes(y = (rate), x = timeBin)) + 
