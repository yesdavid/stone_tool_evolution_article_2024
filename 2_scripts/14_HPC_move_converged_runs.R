...
logf = list.files(path = ".", pattern = "\\.log", recursive = T, full.names = T)
...
for(i in logf){
  ...
  if( ((!ESS_check(log, var = "prior")) || (!ESS_check(log, var = "likelihood")) || (!ESS_check(log, var = "posterior"))) ) {
    ...
    # move output files
    system(paste0("mkdir -p ../done/", paste0( strsplit(i, "/")[[1]][2:4], collapse = "/"), "/ && mv ", j, ".* ../done/", paste0( strsplit(i, "/")[[1]][2:4], collapse = "/"), "/"))
    cat(paste0("mkdir -p ../done/", paste0( strsplit(i, "/")[[1]][2:4], collapse = "/"), "/ && mv ", j, ".* ../done/", paste0( strsplit(i, "/")[[1]][2:4], collapse = "/"), "/\n\n"),
